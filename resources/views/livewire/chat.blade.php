    ...
    <div class="max-w-2xl mx-auto flex flex-col h-full shadow-2xl shadow-zinc-400">
        ...
        @if ($recipient == '')
            <div class="bg-green-700 text-white p-4 text-center">
                <p class="text-xl">Please select a recipient to start chatting.</p>
            </div>

            {{-- Select a person to chat to --}}
            <section class="h-full">
                <div class="p-6 bg-green-50 h-full">
                    <h2 class="text-3xl font-semibold mb-4">Chats</h2>
                    <ul class="space-y-4">
                        @foreach ($users as $user)
                            <li class="flex items-center justify-between p-4 bg-white rounded-lg shadow hover:shadow-lg transition-shadow duration-200">
                                <div class="flex items-center">
                                    @if (file_exists(public_path('images/avatars/' . $user . '.png'))) {{-- If the users avatar exists, pull this, otherwise use a placeholder avatar --}}
                                        <img 
                                            src="{{ asset('images/avatars/' . $user . '.png') }}" 
                                            alt="Avatar"
                                            class="h-10 w-10 rounded-full inline-block mr-4"
                                        >
                                    @else
                                        <div class="h-10 w-10 bg-gray-300 rounded-full inline-block flex justify-center items-center text-center mr-4">
                                            <span class="inline-block mt-2 text-green-900">{{ strtoupper(substr($user, 0, 1)) }}</span>
                                        </div>
                                    @endif
                                    <span class="text-xl">{{ $user }}</span>
                                </div>
                                <button 
                                    wire:click="selectRecipient('{{ $user }}')" 
                                    class="px-4 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 transition-colors duration-200"
                                >
                                    Chat
                                </button>
                            </li>
        ...
        @else
            ...
            <div class="bg-green-800 text-white p-4 flex justify-between items-center">
                ...
                {{-- Back button, Avatar, Name --}}
                    <button class="p-2 text-2xl" wire:click="selectRecipient('')">
                    ...
                    <div class="mb-4">
                        ...
                        @else {{-- If this is NOT an image message --}}
                            ...
                            >
                                <p
                                    class="
                                        border px-4 py-3 rounded-lg text-xl max-w-[80%]
                                ...
                                >
                                    ...
                                    <span 
                                        x-data="{
                                            ...
                                            beginWait() { // This is how long it takes for the recipient to receive the message after being sent
                                                setTimeout(() => {
                                                    this.waited = true
                                    ...
                                    >
                                        @if ($message['user'] === $user)
                                            <i x-show="!waited" class="fa-solid fa-check-double text-gray-500 ml-1 text-sm"></i>
                                            <i x-show="waited" class="fa-solid fa-check-double text-green-500 ml-1 text-sm"></i>
                                        
                                        @else
                                            <i class="fa-solid fa-check-double text-green-500 ml-1 text-sm"></i>
                                        @endif
                                    </span>
                                </p>
                            </div>
